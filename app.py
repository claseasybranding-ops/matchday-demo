...
app = Flask(__name__)
app.secret_key = "matchday_v5_pro_ultra_final"
...
# --- HOVEDLOGIKK FOR RESULTATER OG POENG ---
    ...
    try:
        ...
        for m in res.get('matches', []):
            ...
            if status in ['finished', 'in_play', 'live', 'timed']:
                ...
                # Sjekk om vi allerede har målminuttet lagret
                ...
                if (h_score + a_score) > 0:
                    if stored and stored['first_goal_min']:
                        ...
                        # Hent detaljer kun ved behov for å spare API-kvote og tid
                        try:
...
def group_admin(group_id_str):
    conn = get_db(); c = conn.cursor()
    group = c.execute("SELECT * FROM groups WHERE group_id_str = ?", (group_id_str,)).fetchone()
    alle = c.execute("SELECT * FROM fixtures ORDER BY date ASC").fetchall()
    alle_list = []
    for f in alle:
        f_l = list(f); dt = datetime.fromisoformat(f['date'].replace('Z', '+00:00'))
...
def get_user_bets(group_id_str, user_name):
    ...
    main_bet = [dict(row) for row in c.fetchall()]
    ...
    conn.close()
    return jsonify({'main': [{'h': b['home_score'], 'a': b['away_score'], 'pts': b['points'], 'gg': b['golden_goal'], 'ht': b['home_team'], 'at': b['away_team'], 'hl': b['home_logo'], 'al': b['away_logo']} for b in main_bet], 'extras': [{'q': e['question_text'], 'ans': e['user_answer']} for e in extras]})
