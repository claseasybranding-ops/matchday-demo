...
def update_points_logic():
    ...
    for league in leagues:
        try:
            ...
            res_api = requests.get(url, headers=headers, timeout=10).json()
            if 'matches' not in res_api: continue
            
            for m in res_api['matches']:
                mid = m['id']
...
def get_user_bets(group_id_str, user_name):
    ...
    extras = c.fetchall()
    c.execute("SELECT status FROM fixtures f JOIN group_matches gm ON f.id = gm.fixture_id JOIN groups g ON gm.group_id = g.id WHERE g.group_id_str = ? AND f.status IN ('IN_PLAY', 'LIVE', 'PAUSED')", (group_id_str,))
        ...
        for m in res.get('matches', []):
            ...
            if m_date >= (now - timedelta(hours=3)) and m_date <= future:
                c.execute("INSERT OR REPLACE INTO fixtures (id, league_id, home_team, away_team, home_logo, away_logo, date, status) VALUES (?,?,?,?,?,?,?,?)",
                    (m['id'], league_code, m['homeTeam']['shortName'], m['awayTeam']['shortName'], m['homeTeam'].get('crest',''), m['awayTeam'].get('crest',''), m['utcDate'], m['status']))
        conn.commit(); conn.close()
