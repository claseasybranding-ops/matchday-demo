...
app = Flask(__name__)
app.secret_key = "matchday_final_v4_key"
    ...
    try:
        ...
        for m in res.get('matches', []):
            ...
            if status in ['finished', 'in_play', 'live']:
                ...
                # Finn faktisk minutt for første scoring fra API-detaljer
                ...
                # Regn ut poeng for hver deltaker på denne kampen
                ...
                # Tie-breaker Golden Goal (+2p til nærmeste i hver gruppe)
                if f_goal > 0:
                    c.execute("SELECT DISTINCT group_id_str FROM bets WHERE fixture_id=?", (mid,))
                    for (group_id,) in c.fetchall():
                        c.execute("SELECT MIN(ABS(golden_goal - ?)) FROM bets WHERE fixture_id=? AND group_id_str=? AND golden_goal > 0 AND golden_goal != ?", (f_goal, mid, group_id, f_goal))
                        min_diff = c.fetchone()[0]
                        if min_diff is not None:
                            c.execute("UPDATE bets SET points = points + 2 WHERE fixture_id=? AND group_id_str=? AND ABS(golden_goal - ?) = ?", (mid, group_id, f_goal, min_diff))
        conn.commit(); conn.close()
        return True
    except: return False
...
def leaderboard(group_id_str):
    ...
    c.execute("SELECT * FROM groups WHERE group_id_str = ?", (group_id_str,))
    group = c.fetchone()
    c.execute("SELECT MIN(f.date) FROM fixtures f JOIN group_matches gm ON f.id = gm.fixture_id WHERE gm.group_id = ?", (group[0],))
...
def submit_tips():
    data = request.get_json()
    conn = get_db(); c = conn.cursor()
    # Rens eksisterende tips for denne brukeren i denne gruppa
    c.execute("DELETE FROM bets WHERE group_id_str = ? AND user_name = ?", (data['group_id'], data['user_name']))
    
    # Lagre nye kamp-tips og Golden Goal
    for t in data['tips']:
    ...
    # Lagre tilleggssvar (målscorer etc)
    if 'extras' in data:
